...
@section('content')
    <form action="{{ isset($task) ? route('task.edit', ['task' => $task->id]) : route('tasks.store') }}" method="POST">
        @csrf
        @isset($task)
            @method('PUT')
        @endisset
        <div class="mb-4">
            ...
            <input type="text" @class(['border-red-500' => $errors->has('title')]) name="title" id="titel"
